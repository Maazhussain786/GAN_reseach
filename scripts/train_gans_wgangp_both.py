# train_gans_wgangp_both.py
#
# Train WGAN-GP for BOTH:
#   - Ripe oranges
#   - Unripe oranges
# using the CROPPED dataset (configured in split_dataset.py)
...
# Import loaders & indices from split_dataset (already pointing to orange_crops)
...
# You can reduce to 250–300 if you want faster runs
num_epochs = 300
...
def augment_batch(images, prob=augment_prob):
    """
    Optionally augment a batch with torchvision transforms.
...
# ============================================================
# 6. Training Function (generic for one class)
# ============================================================

def train_wgan_gp(dataloader, label_name, output_dir):
    """
    Train WGAN-GP for a single class (ripe or unripe).
    """
    ...
    print(f"\n{'='*70}")
    print(f"Training WGAN-GP for {label_name.upper()} ORANGES")
    print(f"{'='*70}")
    ...
    print(f"\n{'='*70}")
    print(f"✓ Training complete for {label_name} oranges! Time: {format_time(total_time)}")
    print(f"{'='*70}\n")
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("WGAN-GP Training - CROPPED Orange Dataset (Ripe + Unripe)")
    print("="*70)
    ...
    print("="*70)
    print(f"Ripe   train images: {len(train_ripe_idx)}")
    print(f"Unripe train images: {len(train_unripe_idx)}")
    print("="*70)

    gan_ripe_dir = "./gan_ripe_wgangp_cropped"
    gan_unripe_dir = "./gan_unripe_wgangp_cropped"

    # Train RIPE GAN
    netG_ripe, netD_ripe = train_wgan_gp(ripe_train_loader, "ripe", gan_ripe_dir)

    # Train UNRIPE GAN
    netG_unripe, netD_unripe = train_wgan_gp(unripe_train_loader, "unripe", gan_unripe_dir)

    print("\n" + "="*70)
    print("🎉 BOTH GAN TRAININGS COMPLETE (RIPE + UNRIPE)!")
    print("="*70 + "\n")
